...
% load('lat.mat');
% load('lon.mat');
load('lat_mod.mat'); lat=lat';
load('lon_mod.mat'); lon=lon';
...
masking_threshold=2; %Note, this should be lower than ci_threshold
ci_threshold=5; %12.7 mm 1/2 inch
...
sim_time=30; %12 frames/hr 
start_time='0100'; %Starting Z time string (must be 4 digits)
date='0430'; %date string for saving vars at end
time_plotting_interval=6; %plot every 6 hours
% Grid definitions
nx=364;
ny=375;
%==========================================================================
...
for mem=1:50
    ...
    system(['ln -sf /lustre/scratch/brburgha/fixedphysics/mem' num2str(mem) '/D3fcst/WRFPRS_d01_* ./'])
    for i=1:sim_time
...
%    timediff=abs(master_obslist(yprevstart:yprevend,9)-t);
    ...
    if length(row_inds) > 1
        ...
        clear row_inds
        row_inds=find(dist(:,2)==small,1);%3/14 make sure smallest spatial dist and not time dist;
        %5/15 added ',1' in find to pick 1st if multiple exact min distance        
